...
def recursive_remove_content(repository_pk, content_units):
    ...
    tags_in_repo = Q(pk__in=latest_content.filter(_type='container.tag'))
    manifests_in_repo = Q(pk__in=latest_content.filter(_type='container.manifest'))
    ...
    blobs_in_repo = Q(pk__in=latest_content.filter(_type='container.blob'))
